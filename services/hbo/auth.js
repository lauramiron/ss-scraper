...
async function waitForBrowseReady(page, { timeout = 30000 } = {}) {
  // TODO: Inspect HBO's browse/home page to determine which elements indicate ready state
  // These are educated guesses based on common HBO Max patterns
  const browseSelectors = [
    // Possible navigation elements
    '[data-testid="navigation"]',
    '[data-testid="user-menu"]',
    'nav[class*="navigation"]',
    // Content rail containers
    '[data-testid="content-rail"]',
    '[class*="content-grid"]',
...
export async function ensureLoggedIn(page) {
  ...
  // TODO: Verify how to detect if user is logged in on HBO
  // Check if we're redirected to login page
  const isLoggedIn = !(await page.url()).includes("signIn") && !(await page.url()).includes("login");
  if (!isLoggedIn) {
    await login(page);
  }
